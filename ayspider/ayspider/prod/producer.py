...
#where pro.gameid in (%s) and flag <> 1 order by pri.updatetime limit %s"""
        sql = """select id,gameid,name from product where gameid in (%s) and flag <> 1 limit %d"""
        sql = sql%(self._gameids,self._batcount)
        result = self._sqlconn.fetchall(sql,None)
        tasks = []
        for i in result:
            tasks.append("_%s_%s_%s_%s"%(i['gameid'], 1, i['id'], i['name']))
        return tasks